...
export default function VsMarketPool() {
  ...
  useEffect(() => {
    async function fetchPool() {
      try {
        const response = await fetch(`/api/vsmarket/pool/${id}`);
        if (response.ok) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-lg text-muted-foreground">Loading VS Market pool...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8 p-6 bg-card rounded-2xl border-2 border-transparent bg-gradient-to-br from-neon-green/20 via-transparent to-yellow/20 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
          <div className="flex items-start justify-between flex-wrap gap-4 mb-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-neon-green/30 to-yellow/30 flex items-center justify-center font-mono text-xl font-bold border-2 border-border">
                {pool.token.slice(0, 2)}
              </div>
              <div>
                <h1 className="text-3xl font-bold tracking-tight">
                  {pool.token} <span className="text-muted-foreground text-xl">VS Market</span>
                </h1>
                <p className="text-sm text-neon-green font-semibold mt-1">Community Prediction Market</p>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-6 mb-4">
            ...
            <div className="text-sm">
              ...
              <span className="font-bold text-neon-green">Peer-to-Peer</span>
            </div>
          </div>
        </div>
        ...
        {lockReason && (
          <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/50 rounded-lg">
            <p className="text-sm text-yellow-500">{lockReason}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
              <h2 className="text-lg font-bold tracking-tight mb-4">Price Chart</h2>
              <PriceChart
                data={pool.chart}
                lineBps={null}
                aiLineHistory={null}
            ...
            <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
              <h2 className="text-lg font-bold tracking-tight mb-4">Community Distribution</h2>
