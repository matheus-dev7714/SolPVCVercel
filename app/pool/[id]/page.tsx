...
export default function PoolDetailPage() {
  ...
  useEffect(() => {
    async function fetchPool() {
      try {
        const response = await fetch(`/api/pool/${id}`, { cache: "no-store" })
        if (response.ok) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-lg text-muted-foreground">Loading pool...</div>
      </div>
    )
  }
        ...
        <div className="mb-8 p-6 bg-card rounded-2xl border-2 border-transparent bg-gradient-to-br from-aqua/20 via-transparent to-neon-purple/20 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
          <div className="flex items-start justify-between flex-wrap gap-4 mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold tracking-tight">
                  {pool.token} <span className="text-muted-foreground text-xl">Daily Pool</span>
                </h1>
            ...
            <div className="text-right">
              ...
              <Timer lockTs={pool.lock_ts} endTs={pool.end_ts} status={pool.status} hideLabel />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
              ...
              <PriceChart data={pool.chart} lineBps={pool.line_bps} aiLineHistory={pool.ai_line_history} poolType={pool.pool_type} poolId={pool.id} />
            </div>
